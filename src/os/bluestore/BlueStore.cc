...
void BlueStore::BufferSpace::split(BufferCacheShard *cache, size_t pos, BlueStore::BufferSpace &r)
{
    ...
    while (true)
    {
        ...
        if (p->second->data.length())
        {
            r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->data),
                          0, p->second.get());
        }
        else
        {
            r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->length),
                          0, p->second.get());
        }
...
public:
    ...
    struct FSCKWorkQueue : public ThreadPool::WorkQueue_
    {
        ...
        FSCKWorkQueue(std::string n,
                      ...
                      BlueStoreRepairer *_repairer) : WorkQueue_(n, time_t(), time_t()),
...
int BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair)
{
    dout(1) << __func__
            << (repair ? " repair" : " check")
            << (depth == FSCK_DEEP ? " (deep)" : depth == FSCK_SHALLOW ? " (shallow)" : " (regular)")
            << dendl;
...
void BlueStore::_get_statfs_overall(struct store_statfs_t *buf)
{
    ...
    if (bluefs)
    {
        ...
        buf->internal_metadata =
            std::max(bluefs->get_used(), (uint64_t)cct->_conf->bluestore_bluefs_min) - buf->omap_allocated;
    }
...
void RocksDBBlueFSVolumeSelector::dump(ostream &sout)
{
    ...
    constexpr std::array<const char *, 7> names{{"DEV/LEV",
                                                 "WAL",
                                                 "DB",
                                                 "SLOW",
                                                 "*",
